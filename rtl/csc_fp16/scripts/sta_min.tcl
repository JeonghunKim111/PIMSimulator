# Required environment: NETLIST, TOP, SDC, LIB_MIN.
...
set liberty [require_env LIB_MIN]
foreach path [list $liberty $netlist $sdc] {
  if {![file isfile $path]} { puts stderr "error: input file not found: $path"; exit 2 }
}
read_liberty -min $liberty
read_verilog $netlist
link_design $::env(TOP)
read_sdc $sdc
check_setup
report_checks -path_delay min -group_path_count 10 -endpoint_path_count 10 -fields {slew cap input_pin}
