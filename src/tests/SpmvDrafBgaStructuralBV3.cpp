    ...
    double gpu_ms, double target_speedup, double tck_ns)
{
    ...
    /*
     * BV-3 keeps the BV-2 runtime-orchestration base and adds movement terms
     * that BV-2 intentionally left hidden inside the ideal DRAF/BGA pipeline.
     *
     * - fragmented_draf_replay_cycle: DRAF packing expansion and hidden
     *   padding are not always fully overlapped. Fragmented row groups cause
     *   replay-like operand movement on top of the useful compute trigger.
     * - low_density_operand_access_cycle: sparse columns with few nnz still
     *   consume command/address traversal bandwidth, so very low density
     *   columns pay an access overhead per DRAF column group.
     * - skewed_partial_movement_cycle: BGA partials are cheap only when row
     *   reuse is balanced and local. Row/column skew and low reuse increase
     *   partial movement before accumulation.
     *
     * These additions are structural features of the packed DRAF layout and
     * BGA reduction stream; they do not use target speedup or matrix names.
     */
    ...
    cout << "  latency_scope: BV-2 + fragmented DRAF replay"
         << " + low-density operand access + skewed partial movement" << endl;
    ...
    cout << variant_spec.result_csv_tag
         ...
         << fragmented_draf_replay_cycle << ","
         << low_density_operand_access_cycle << ","
         << skewed_partial_movement_cycle << endl;
