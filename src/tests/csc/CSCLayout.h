...
struct CSCLayout{CSCMatrix matrix;MappingPolicy mapping_policy;std::vector<uint32_t> column_to_bg;std::vector<BGImage> bg;LayoutStats stats;};
CSCLayout buildLayout(const CSCMatrix&,MappingPolicy,const std::vector<uint32_t>& external={});
void validateLayout(const CSCLayout&);std::string mappingPolicyName(MappingPolicy);
}
#endif
