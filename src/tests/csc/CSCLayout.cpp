...
CSCLayout buildLayout(const CSCMatrix&m,MappingPolicy p,const std::vector<uint32_t>&ext){
 ...
 refreshLayoutStats(l);validateLayout(l);return l;
}
void refreshLayoutStats(CSCLayout&l){auto&m=l.matrix;auto&s=l.stats;s={};s.num_rows=m.rows;s.num_cols=m.cols;s.nnz=m.values.size();s.bg_nnz.assign(64,0);s.bg_descriptor_count.assign(64,0);
 ...
 s.useful_value_bytes=s.nnz*4;s.useful_index_bytes=s.nnz*4;s.value_alignment_padding_bytes=s.physical_value_bytes-s.useful_value_bytes;s.index_alignment_padding_bytes=s.physical_index_bytes-s.useful_index_bytes;s.total_alignment_padding_bytes=s.value_alignment_padding_bytes+s.index_alignment_padding_bytes;uint64_t useful=s.useful_value_bytes+s.useful_index_bytes;s.alignment_overhead_ratio=useful?double(s.total_alignment_padding_bytes)/useful:0;s.descriptor_bytes=s.descriptor_count*32;s.average_bg_nnz=double(s.nnz)/64;double var=0;for(auto n:s.bg_nnz)var+=(n-s.average_bg_nnz)*(n-s.average_bg_nnz);s.load_cv=s.average_bg_nnz?std::sqrt(var/64)/s.average_bg_nnz:0;
}
