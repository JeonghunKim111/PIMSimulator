...
class CSCTimingModel{public:CSCTimingModel();TimingStats run(const CSCLayout&,const ExecutionStats&,bool result=true);uint64_t globalBGCount()const;private:uint64_t address(uint32_t,uint32_t,uint32_t,uint64_t);uint64_t drain();uint64_t image(const CSCLayout&,bool,bool,uint32_t,uint32_t,const char*);std::shared_ptr<DRAMSim::MultiChannelMemorySystem> mem_;std::shared_ptr<PIMAddrManager> am_;unsigned channels_=0,ranks_=0,bgs_=0,banks_=0;uint64_t cycle_=0;DRAMSim::BurstType burst_;};
}
#endif
