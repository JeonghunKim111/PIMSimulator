    ...
    double gpu_ms, double target_speedup, double tck_ns)
{
    ...
    /*
     * V18 keeps the V17-1 regular-leakage gate and adds a graph-like
     * critical sync exposure term for irregular padding/tail latency.
     */
    ...
    if (regular_leakage_score > 0.0)
    {
        regular_soft_alpha_cap =
            clamp01(0.20 - 0.18 * regular_leakage_score +
    ...
    uint64_t protected_bga_hidden = static_cast<uint64_t>(
        floor(static_cast<double>(bga_hidden_demand) *
              ...
              (1.0 - 0.85 * regular_leakage_score *
    ...
    double graph_sync_gate =
        clamp01((graph_like_penalty_score - 0.68) / 0.25);
    double graph_tail_gate =
        clamp01((1.0 - critical.tail_exposure_ratio) / 0.45);
    double graph_regular_guard =
        1.0 - clamp01(regular_leakage_score / 0.70);
    uint64_t graph_tail_pool_cycle = static_cast<uint64_t>(
        ceil(static_cast<double>(critical.p95_to_max_tail_steps) *
             static_cast<double>(kElementsPerBurst)));
    uint64_t graph_tail_sync_exposure_cycle = static_cast<uint64_t>(
        ceil(static_cast<double>(graph_tail_pool_cycle) *
             graph_sync_gate * graph_tail_gate * graph_regular_guard * 0.55));
    uint64_t graph_padding_sync_exposure_cycle = static_cast<uint64_t>(
        ceil(static_cast<double>(exposed_padding_cycle) *
             graph_sync_gate * graph_tail_gate * graph_regular_guard * 0.20));
    uint64_t graph_sync_exposure_cycle =
        graph_tail_sync_exposure_cycle + graph_padding_sync_exposure_cycle;
    uint64_t padding_path_cycle =
        exposed_padding_cycle > bounded_padding_saved
            ? exposed_padding_cycle - bounded_padding_saved
            : 0;
    padding_path_cycle += graph_padding_penalty;
    padding_path_cycle += graph_sync_exposure_cycle;
    ...
    result.v18_soft_alpha = soft_alpha;
    result.v18_true_high_speedup_candidate_score = true_high_speedup_candidate_score;
    result.v18_regular_false_positive_score = regular_false_positive_score;
    result.v18_graph_like_penalty_score = graph_like_penalty_score;
    result.v18_regular_leakage_score = regular_leakage_score;
    result.v18_high_confidence_protection_score = high_confidence_protection_score;
    result.v18_regular_soft_alpha_cap = regular_soft_alpha_cap;
    result.v18_graph_sync_gate = graph_sync_gate;
    result.v18_graph_tail_gate = graph_tail_gate;
    result.v18_graph_sync_exposure_cycle = graph_sync_exposure_cycle;
    result.v18_saving_budget_cycle = saving_budget;
    result.v18_saving_demand_cycle = saving_demand;
    result.v18_effective_saving_cycle = effective_saving;
    result.v18_protected_bga_hidden_cycle = protected_bga_hidden;
    result.v18_final_bounded_bga_hidden_cycle = bounded_bga_hidden;
    ...
    cout << "  latency_scope: setup + DRAF pipeline path + regular-leakage-gated"
         << " soft saving budget + reuse-protected BGA + graph critical sync exposure" << endl;
    ...
    cout << "> soft_saving_budget: budget=" << saving_budget
         ...
         << " saturation_ratio=" << result.v18_budget_saturation_ratio << endl;
    ...
    cout << "> padding_path_cycle: " << padding_path_cycle
         ...
         << " graph_padding_penalty=" << graph_padding_penalty
         << " graph_sync_gate=" << graph_sync_gate
         << " graph_tail_gate=" << graph_tail_gate
         << " graph_sync_exposure_cycle=" << graph_sync_exposure_cycle << endl;
    ...
    cout << variant_spec.result_csv_tag
         ...
         << bounded_bga_hidden << "," << result.v18_budget_saturation_ratio
         << endl;
    return result;
}
