...
const char* modeName(CSCExecutionMode m){switch(m){case CSCExecutionMode::COMPUTE_ONLY:return "COMPUTE_ONLY";case CSCExecutionMode::BGA_VALIDATION:return "BGA_VALIDATION";case CSCExecutionMode::TRANSPORT_ONLY:return "TRANSPORT_ONLY";case CSCExecutionMode::END_TO_END_TIMED:return "END_TO_END_TIMED";}throw std::invalid_argument("invalid CSC execution mode");}
...
uint32_t validationAcceptsPerCycle(){const char* text=std::getenv("CSC_BGA_VALIDATION_ACCEPTS_PER_CYCLE");if(!text||!*text)return 64;char*end=nullptr;unsigned long value=std::strtoul(text,&end,10);if(*end||!value||value>64)throw std::invalid_argument("CSC_BGA_VALIDATION_ACCEPTS_PER_CYCLE must be in [1,64]");return uint32_t(value);}
uint32_t bgaCapacity(){const char* text=std::getenv("CSC_FP16_BGA_CAPACITY");if(!text||!*text)return 64;char*end=nullptr;unsigned long value=std::strtoul(text,&end,10);if(*end||!value||value>UINT32_MAX)throw std::invalid_argument("CSC_FP16_BGA_CAPACITY must be a positive uint32");return uint32_t(value);}
}
...
CSCFp16M7Result runFp16M7(std::shared_ptr<const CSCFp16ExecutionImage> image,CSCExecutionMode mode,uint64_t max_cycles)
{
    ...
    CSCFp16M7Result result;result.mode=mode;const auto&i=image->image();const uint32_t bga_capacity=mode==CSCExecutionMode::COMPUTE_ONLY?16:bgaCapacity();result.configuration_preset=mode==CSCExecutionMode::COMPUTE_ONLY?"FP16_COMPUTE_ONLY":"FP16_BATCH8_Q"+std::to_string(bga_capacity);result.rows=i.matrix.rows;result.columns=i.matrix.cols;result.nnz=i.matrix.value_bits.size();result.matrix_fingerprint=matrixFingerprint(i);result.mapping_fingerprint=mappingFingerprint(i);result.preflight=preflightFp16ResultRegion(*image);for(const auto&b:result.preflight.bg){result.descriptor_count+=b.descriptor_count;if(b.descriptor_count)result.active_bg_count++;}
    ...
    auto bga=makeFp16IsoStructureProductionConfig(i.matrix.rows);bga.input_queue_depth=bga_capacity;bga.accumulator_entries=bga_capacity;bga.compare_width=bga_capacity;bga.output_queue_depth=bga_capacity;std::size_t output_cap=1;for(const auto&b:result.preflight.bg)output_cap=std::max<std::size_t>(output_cap,b.upper_bound_output_records+1);
    if(mode==CSCExecutionMode::BGA_VALIDATION){CSCFp16NativeExecution execution(image,output_cap,&bga,output_cap,nullptr,validationAcceptsPerCycle());drive(execution,max_cycles);const auto c=execution.counters();result.compute_complete_cycle=c.timing.compute_complete_cycle;result.bga_complete_cycle=c.timing.compute_bga_completion_cycle;std::vector<CSCFp16BGAOutputEvent> events;for(uint32_t bg=0;bg<64;++bg){const auto&t=execution.bgaOutputSink(bg).trace();events.insert(events.end(),t.begin(),t.end());}auto reduced=reduceCapturedFp16BGAOutputs(i.matrix.rows,events);result.bga_output_count=events.size();result.bga_output_hash=cscFp16BGAOutputTraceFnv1a64(events);result.host_add_count=reduced.host_add_count;result.rows_touched=reduced.rows_touched;result.final_y_hash=reduced.final_y_hash;result.final_y_bits=std::move(reduced.final_y_bits);return result;}
    if(mode!=CSCExecutionMode::TRANSPORT_ONLY&&mode!=CSCExecutionMode::END_TO_END_TIMED)
        throw std::invalid_argument("unsupported FP16 M7 mode");
    CSCFp16TransportConfig transport;transport.host_reduction_enabled=mode==CSCExecutionMode::END_TO_END_TIMED;uint64_t max_bursts=1;for(const auto&b:result.preflight.bg)max_bursts=std::max(max_bursts,b.allocated_result_bursts);if(max_bursts>std::numeric_limits<uint32_t>::max())throw std::overflow_error("FP16 result region too large");transport.buffer_capacity_bursts_per_bg=uint32_t(max_bursts);CSCFp16NativeExecution execution(image,output_cap,&bga,output_cap,&transport);drive(execution,max_cycles);const auto c=execution.counters();const auto&t=execution.transport();const auto&tc=t.counters();result.compute_complete_cycle=c.timing.compute_complete_cycle;result.bga_complete_cycle=c.timing.compute_bga_completion_cycle;result.writeback_complete_cycle=tc.writeback_complete_cycle;result.readback_complete_cycle=tc.readback_complete_cycle;if(transport.host_reduction_enabled){result.host_reduction_complete_cycle=tc.reduction_complete_cycle;result.end_to_end_cycle=tc.end_to_end_cycle;}result.bga_output_count=tc.outputs_accepted;result.bga_output_hash=cscFp16BGAOutputTraceFnv1a64(canonicalEvents(t.acceptedTrace()));uint64_t contributions=0;for(const auto&e:t.acceptedTrace())contributions+=e.contribution_count;if(contributions!=result.nnz)throw std::runtime_error("FP16 BGA contribution conservation mismatch");result.bga_contribution_count=contributions;result.transport_record_count=tc.records_packed;result.write_bursts=tc.writes_accepted;result.read_bursts=tc.reads_accepted;result.write_bytes=tc.write_bytes;result.read_bytes=tc.read_bytes;result.padding_bytes=tc.tail_padding_bytes;result.transport_stall_cycles=tc.transport_stall_cycles;result.host_add_count=tc.fp16_host_adds;result.rows_touched=tc.rows_touched;if(transport.host_reduction_enabled){result.final_y_bits=execution.finalYFp16Bits();result.final_y_hash=cscFp16FinalYFnv1a64(result.final_y_bits);}return result;
}

std::string CSCFp16M7Result::toJson() const
{
    std::ostringstream s;s<<"{\"precision\":\"FP16\",\"execution_mode\":\""<<modeName(mode)<<"\",\"configuration_preset\":\""<<configuration_preset<<"\",\"rows\":"<<rows<<",\"columns\":"<<columns<<",\"nnz\":"<<nnz<<",\"matrix_fingerprint\":\""<<std::hex<<matrix_fingerprint<<"\",\"mapping_fingerprint\":\""<<mapping_fingerprint<<std::dec<<"\"";auto field=[&](const char*n,const std::optional<uint64_t>&v){if(v)s<<",\""<<n<<"\":"<<*v;};field("compute_complete_cycle",compute_complete_cycle);field("bga_complete_cycle",bga_complete_cycle);field("writeback_complete_cycle",writeback_complete_cycle);field("readback_complete_cycle",readback_complete_cycle);field("host_reduction_complete_cycle",host_reduction_complete_cycle);field("end_to_end_cycle",end_to_end_cycle);field("bga_output_count",bga_output_count);field("bga_contribution_count",bga_contribution_count);field("transport_record_count",transport_record_count);field("write_bursts",write_bursts);field("read_bursts",read_bursts);field("write_bytes",write_bytes);field("read_bytes",read_bytes);field("padding_bytes",padding_bytes);field("transport_stall_cycles",transport_stall_cycles);field("host_add_count",host_add_count);field("rows_touched",rows_touched);field("final_y_hash",final_y_hash);s<<'}';return s.str();
}
...
void publishFp16M7Artifacts(const CSCFp16M7Result&r,const std::string&directory)
{
    ...
    {std::ofstream f(tmp/"traffic.csv");f<<"metric,value\n";auto row=[&](const char*n,const std::optional<uint64_t>&v){if(v)f<<n<<','<<*v<<'\n';};row("transport_records",r.transport_record_count);row("write_bursts",r.write_bursts);row("read_bursts",r.read_bursts);row("write_bytes",r.write_bytes);row("read_bytes",r.read_bytes);row("padding_bytes",r.padding_bytes);row("transport_stall_cycles",r.transport_stall_cycles);}
    {std::ofstream f(tmp/"bga_stats.csv");f<<"metric,value\n";if(r.bga_output_count)f<<"output_records,"<<*r.bga_output_count<<'\n';if(r.bga_contribution_count)f<<"input_contributions,"<<*r.bga_contribution_count<<'\n';if(r.bga_output_hash)f<<"output_trace_hash,"<<std::hex<<*r.bga_output_hash<<std::dec<<'\n';}
    ...
    if(!r.final_y_bits.empty()){std::ofstream f(tmp/"final_y_fp16.bin",std::ios::binary);for(auto bits:r.final_y_bits){char b[2]={char(bits),char(bits>>8)};f.write(b,2);}}
    fs::rename(tmp,out);
}
